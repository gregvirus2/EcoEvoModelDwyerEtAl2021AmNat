...
int ExtRlzns = 0;
int Count = 0;
int DIC = 0; //NOT DIC, INSTEAD ITS PLOTTING MODEL VS DATA
int ModelVsData = 0; //DIC must also equal 1! AND NiceVerbose!
int NiceVerbose = 0;
int Evoln = 1;   //Search on .txt
int WAIC = 1;
int LineSearch = 0;
int sampSize = 6; 
int PopWiseNumSamp;
...
//double kIn[5000],nuIn[5000],muIn[5000],ratioIn[5000],deltaIn[5000],mIn[5000],
	...
	//printf("Pars1:%f Pars2:%f Pars4:%f Pars5:%f deltaIn:%f Pars9:%f \n",Pars[1],Pars[2],Pars[4],Pars[5],deltaIn,Pars[9]); getc(stdin);
	...
	*/
	...
	//2.509875	Fxd7.134631	0.06196643	1.337532	0.7357414	0.1849719	0.1923509	2.752323	0.01794659	0.05647819	3.491694
	//0.5136258	5.05446	7.917562	0.06623353	0.7357414	0.1849719	0.1923509	2.752323	0.01794659	0.05647819	3.491694
	...
	if(Evoln == 1){
		...
		if(ModelVsData==1){	 //These parameters are from AllMEG3B28O28YA.dat and G3A8O28YA.in, and have output in NiceCycles13July16.dat	
			phi = 7.917562;
        		a = 0.06623353;
        		sigma =1.337532;
			r = 0.51;
		...
		//printf("r:%f s:%f phi:%f w:%f b:%f sigma:%f \n",r,s,phi,a,b,sigma); getc(stdin);
	}else{
		phi = FxdPars[2];
		a = FxdPars[3];
		sigma = FxdPars[4];
		//printf("No Evoln r:%f s:%f phi:%f w:%f sigma:%f \n",r,s,phi,a,sigma); getc(stdin);

		//1.569974	7.071491	0.069629	2.658093	0.7357414	0.1849719	0.1923509	2.752323	0.01794659	0.05647819	3.491694
	...
	//printf("Ntemp:%f Ztemp:%f nutemp:%f\n",Ntemp,Ztemp,nutemp); //getc(stdin);
	...
	Infnu = nu = oldInfnu = oldnu = nutemp;
	  ...
	  if(Dump!=1){
		...
		if(Evoln<1){
		 ...
		 //printf("n:%d N:%e Dump:%d sigma:%f\n",n,N,Dump,sigma); getc(stdin);
		...
		if(sigma>0.0){ //STOCHASTICITY FOR NON-BURNOUT
		  ...
		  //printf("n:%d N:%e Dump:%d exp(gauss):%f sigma:%f \n",n,N,Dump,exp(gauss),sigma); getc(stdin);
		}
		...
		//printf("n:%d Dump:%d oldN:%e oldZ:%e \n",n,Dump,oldN,oldZ);
...
/* 17 December 2019 */
	...
	fp = fopen("GA8O42YC.in","r"); //GM, but check to make sure rows are ok!
...
//Informative
   PropMean[1] = log(0.22); //log(40.0); //log(25.0); //r, see NatSel16, SI, page 52
   PropVar[1] =  log(0.45/2.0); //0.25; //0.25; //MN2A: 5.0 OFN: 1.5; //OFM: 1;
   PropDist[1] = 2; //normal-log
...
*/
  PropDist[2] = 2; //s - slope
  PropMean[2] = log(1.21);  //NatSel16, SI, page 52
...
//4, which is a, which is the density at which predation is highest - THIS IS DIFFERENT THAN DWYER ET AL. 2004!
...
//3, which is a, the density at which predation is maximized, NOTE THAT THIS DIFFERENT THAN DWYER ET AL 2004!
  //Here are the proposals
	...
	char *test[] =
	  {
	    ...
	    "LSNE8O42YC", //Line Search
	    "MEG48O28YA", //"NDOAC5FThin1e2Seed", //"LGx5O5JThin1e2", //"LDOAC5FThin1e2Seed", //f"NGM4FESeed"
	  };
...
if(LineSearch==1){
 ...
 if(Evoln<1){
	...
	Lower[1] = 0.05; Upper[1] = 2.5; Jump[1] = 0.2; //lambda
 	Lower[2] = 1.5; Upper[2] = 2.5; Jump[2] = 0.1; //phi
	Lower[3] = -4; Upper[3] = 0.0; Jump[3] = 0.5; //a
	Lower[4] = -3; Upper[4] = 2.0; Jump[4] = 1.0; //sigma
	FxdPars[5] = 0.0;
	FxdPars[6] = 1.0;
	...
	Lower[1] = -2; Upper[1] = 0; Jump[1] = 0.5; //r
 	Lower[2] = -2; Upper[2] = 1.5; Jump[2] = 0.5; //s
 	Lower[3] = 0.5; Upper[3] = 3.0; Jump[3] = 0.5; //phi
	Lower[4] = -4; Upper[4] = 0.0; Jump[4] = 0.5; //a
	Lower[5] = -3; Upper[5] = 2.0; Jump[5] = 1; //sigma
	Lower[6] = -4; Upper[6] = 0.0; Jump[6] = 0.5; //b
...
//FxdPars[1] = 0.3678794; FxdPars[2] =  7.389056; FxdPars[3] = 1.254735e1; FxdPars[4] = 1.175062e-1; FxdPars[5] = 2.718282;
//3.678794e-01 7.389056e+00 1.254735e+01 1.175062e-01 2.718282e+00 -1.060521e+01 -2.381138e+01
			...
			//printf("done with LH Calc...maxBoots:%d AvgLH:%f\n",maxBoots,log(AvgLH)); //getc(stdin);
     ...
     for(PC=1;PC<=NumParams;PC++){
      ...
      for(LStemp=Lower[PC];LStemp<=Upper[PC];LStemp+=Jump[PC]){
		...
		//printf("LSItn:%d k:%f nubar:%f mu:%f sigma:%f ratio:%f delta:%f m:%f gamma:%f LHoodPerPop:%f OldLHoodPerPop:%f NewPost:%f OldPost:%f\n",LSItn,ParamVals.FxdPars[1],ParamVals.FxdPars[2],ParamVals.FxdPars[3],ParamVals.FxdPars[4],ParamVals.FxdPars[5],ParamVals.FxdPars[6],ParamVals.FxdPars[9],ParamVals.FxdPars[20],AvgLH,OldAvgLH,NewPost,OldPost);	
				...
				//if(PC==6) 
				//printf("LSItn:%d PC:%d LStemp:%f FxdPars1:%f FxdPars2:%f FxdPars3:%f FxdPars4:%f FxdPars5:%f FxdPars6:%f OldAvgLH:%f AvgLH:%f NewPost:%f OldPost:%f\n",LSItn,PC,LStemp,FxdPars[1],FxdPars[2],FxdPars[3],FxdPars[4],FxdPars[5],FxdPars[6],OldAvgLH,AvgLH,NewPost,OldPost);
				//getc(stdin);
			} else{ 
				//LSflag = 0;
...
if(DIC==1){
	...
	if(Evoln==1){
		...
		printf("FxdPars:%f\n",FxdPars[1]);
		double junk;
		//&kIn[SEIRParamNum],&nuIn[SEIRParamNum],&muIn[SEIRParamNum],&sigmaIn[SEIRParamNum],&ratioIn[SEIRParamNum],&deltaIn[SEIRParamNum],&Error
...
*/
	...
	double *rIn, *sIn, *phiIn, *wIn, *sigIn, *bIn;
	const NumSets = 10000000;
	kIn = dvector(0,NumSets);
	nuIn = dvector(0,NumSets);
	muIn = dvector(0,NumSets);
...
/*************** Change file name here ******************/
	...
	if(Evoln==1){
			while(fscanf(fp,"%lf %lf %lf %lf %lf %lf\n",&rIn[DLParamNum],&sIn[DLParamNum],&phiIn[DLParamNum],&wIn[DLParamNum],&sigIn[DLParamNum],&bIn[DLParamNum])!=EOF){
			
				printf("%d %f %f %f %f %f %f\n",DLParamNum,rIn[DLParamNum],sIn[DLParamNum],phiIn[DLParamNum],wIn[DLParamNum],sigIn[DLParamNum],bIn[DLParamNum]);
				//getc(stdin);
				DLParamNum++;	
			}
	} else{
			...
			while(fscanf(fp,"%lf %lf %lf %lf\n",&rIn[DLParamNum],&phiIn[DLParamNum],&wIn[DLParamNum],&sigIn[DLParamNum])!=EOF){

				sIn[DLParamNum] = 0.0;
			
				printf("%d %f %f %f %f %f\n",DLParamNum,rIn[DLParamNum],sIn[DLParamNum],phiIn[DLParamNum],wIn[DLParamNum],sigIn[DLParamNum]); 
				//getc(stdin);
				DLParamNum++;	
			}
	} //Evoln
...
for(DLParamNum = 0;DLParamNum<MaxDLParamNum;DLParamNum++){
	...
	if(Evoln==1){
		...
		FxdPars[4] = wIn[DLParamNum];
		FxdPars[5] = sigmaIn[DLParamNum];
		FxdPars[6] = bIn[DLParamNum];
	}else{
		FxdPars[2] = phiIn[DLParamNum];
		FxdPars[3] = wIn[DLParamNum];
		FxdPars[4] = sigmaIn[DLParamNum];
...
//Calculate the likelihood	
	...
	//maxBoots = 25;
			...
			for(boot=0;boot<maxBoots;boot++){
				...
				if(res>-1e10){
					...
					//printf("DLParamNum:%d boot:%d res:%f OldAvgLH:%e PopWiseNumSamp:%d\n",DLParamNum,boot,res,OldAvgLH,PopWiseNumSamp);
					// getc(stdin);
			...
			//printf("DLParamNum:%d FXd1:%f Fxd2:%f FXd3:%f Fxd4:%f Fxd5:%f Fxd6:%f bootGood:%d OldPost:%f\n",DLParamNum,FxdPars[1],FxdPars[2],FxdPars[3],FxdPars[4],FxdPars[5],FxdPars[6],bootGood,OldPost);
			//getc(stdin);


 	} //DLParamNum

	fp = fopen("DLWAIC2019.dat","a");
	...
	fprintf(fp,"MNE8O42YC TotPopWiseMeanLog:%f pWAIC2B:%f AltWAIC:%f \n",TotPopWiseMeanLog,pWAIC2B,AltWAIC);
	fclose(fp);
...
///////////////////////////////   Reading in PCA Results ///////////////////////////
	...
	if(Evoln<1) ParamVals.FxdPars[5] = FxdPars[5] = CurrentPars[5] = 0.0;

	
	//printf("ParCnt2:%d\n",ParCnt2); getc(stdin);


	//fp = fopen("LSNEG48O28YARotations.txt", "r");
	//fp = fopen("LSE8O42YCRotations.txt", "r");
	fp = fopen("LSNE8O42YCRotations.txt", "r");
	...
	//fp = fopen("LSNEG48O28YAScale.txt", "r");
	//fp = fopen("LSE8O42YCScale.txt", "r");
	fp = fopen("LSNE8O42YCScale.txt", "r");
	...
	//fp = fopen("LSNEG48O28YACenter.txt", "r");
	//fp = fopen("LSE8O42YCCenter.txt", "r");
	fp = fopen("LSNE8O42YCCenter.txt", "r");
	...
	//fp = fopen("LSNEG48O28YAsd.txt", "r");
	//fp = fopen("LSE8O42YCsd.txt", "r");
	fp = fopen("LSNE8O42YCsd.txt", "r");
...
//FxdPars[1] = 0.3678794; FxdPars[2] =  0.1353353; FxdPars[3] = 7.389056; FxdPars[4] = 0.08208500; FxdPars[5] = 2.718282;
//FxdPars[1] = 0.3678794; FxdPars[2] =  7.389056; FxdPars[3] = 1.254735e1; FxdPars[4] = 1.175062e-1; FxdPars[5] = 2.718282;
//FxdPars[1] = 2.0641; FxdPars[2] =  7.78692; FxdPars[3] = 5.195292e-2; FxdPars[4] = 2.254797; 

//1171.000000  2.064074e+00  7.786920e+00  5.195292e-02  2.254797e+00  -1.214272e+01 -1.714647e+01
	...
	//printf("MCMC LH calc finished...boot:%d bootGood:%d OldAvgLH:%f\n",boot,bootGood,OldAvgLH);  
	//exit(1);
	 ...
	 ////////////// Calculate likelihood ///////////////
		...
		if(Stop<1){
			...
			LHoodPerPop = 0.0;
					...
					//printf("boot:%d bootGood:%d OldAvgLH:%f\n",boot,bootGood,log(AvgLH));

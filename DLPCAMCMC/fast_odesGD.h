...
//printf("k:%f nu:%f mu:%f delta:%f m:%d S0:%f y[0]:%f y[m+1]:%f\n",k,nu,mu,delta,m,S0,y[0],y[m+1]); getc(stdin);
...
//double hetero = 1.124951*nu*pow( ((double) (y[0]/S0)), ((double) V));
// ------------------------------------------ ODEs -------------------------------------------- //
...
//printf("t:%f dydt[0]:%f k:%f nu:%f mu:%f delta:%f m:%d S0:%f y[0]:%f y[m+1]:%f V:%f\n",t,dydt[0],k,nu,mu,delta,m,S0,y[0],y[m+1],V); getc(stdin);
...
//sys_ode.jacobian  = jacobian; //NOT NEEDED FOR THIS NUMERICAL INTEGRATION ALGORITHM
